...
function getPriceForModel(model) {
    ...
    return {
        input: raw.input / 1000000,
        output: raw.output / 1000000,
        cacheRead: raw.cacheRead / 1000000,
        cacheCreation: raw.cacheCreation / 1000000,
    };
}
...
function readConfigDailyLimit(projectRoot) {
    try {
...
function sumLines(lines, todayPrefix) {
...
function getUsage(projectRoot) {
    try {
        ...
        const withStat = files.map((f) => ({ f, mtime: fs.statSync(path.join(dir, f)).mtimeMs }));
...
function formatStatusBar(memory) {
...
function getChecklistHtml(memory) {
...
function gaugeHtml(name, val, target) {
...
function activate(context) {
    ...
    usageTimer = setInterval(() => updateStatusBar(), 30000);
    context.subscriptions.push({ dispose: () => { if (usageTimer)
    ...
    // 커맨드: 체크리스트 패널
    context.subscriptions.push(vscode.commands.registerCommand('coat.showChecklist', () => {
        const memPath = getMemoryPath();
        const memory = memPath ? readMemory(memPath) : {};
        const panel = vscode.window.createWebviewPanel('coatChecklist', `COAT — ${memory.feature || '현황'}`, vscode.ViewColumn.Beside, { enableScripts: false });
        panel.webview.html = getChecklistHtml(memory);
    }));
    // 커맨드: 대시보드 열기
    context.subscriptions.push(vscode.commands.registerCommand('coat.openDashboard', () => {
        vscode.env.openExternal(vscode.Uri.parse('http://localhost:3030'));
    }));
    // 커맨드: 상태바 클릭 메뉴 (체크리스트 / 대시보드)
    context.subscriptions.push(vscode.commands.registerCommand('coat.showMenu', async () => {
