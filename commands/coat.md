...
classification-reason: COAT personal development methodology cycle automation
...
1. p1-record 파일 저장:
   ```
   .coat/audit/p1-records/{이슈명}-{YYYY-MM-DD}.md
     내용:
       상태: ✅ 해결 / ❌ 실패
       시도별 방법 + 결과 + 실패 이유
...
2. `_INDEX.md` 자동 생성/갱신:
   - 파일 없으면 신규 생성, 있으면 항목 추가
   - 포맷:
     ```markdown
     # P1* 처리 기록
     ...
     | 이슈명 | 날짜 | 결과 | 라운드 |
     |--------|------|------|--------|
     | {이슈명} | {YYYY-MM-DD} | ✅ 해결 / ❌ 실패 | R{N} |

     **해결률: {해결 수} / {전체 수} ({퍼센트}%)**
     ```
   - 저장 후: `"_INDEX.md 갱신됨: .coat/audit/p1-records/_INDEX.md"`
...
/coat github on
  ...
  4. .coat/config.json 저장 (Bash: node -e 또는 직접 JSON 수정)
  5. 라벨 자동 생성 (2>/dev/null 으로 이미 존재해도 무시):
